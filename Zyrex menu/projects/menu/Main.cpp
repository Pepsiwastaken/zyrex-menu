...
// ─────────────────────────────────────────────────────────────────────────
//  Robust FDM pointer discovery.
//  The FDM/VE "Pointer" RVAs from the auto-updated dump can be wrong for
//  some client builds (they must land inside the game module's image). If
//  the static read fails we scan the module images for a QWORD slot whose
//  FDM -> RealDataModel -> Players chain validates, and override the offset.
// ─────────────────────────────────────────────────────────────────────────
struct mod_region_t { uint64_t start; uint64_t size; };

static std::vector<mod_region_t> BuildReadableRegions()
{
    std::vector<mod_region_t> regions;
    uint64_t addr = 0;
    MEMORY_BASIC_INFORMATION mbi{};
    while (addr < 0x7FFFFFFF0000ULL)
    {
        if (VirtualQueryEx(memory->get_process_handle(), (LPCVOID)addr, &mbi, sizeof(mbi)) == 0)
            break;
        if (mbi.State == MEM_COMMIT &&
            (mbi.Protect & (PAGE_READONLY | PAGE_READWRITE | PAGE_WRITECOPY |
                            PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY)))
        {
            regions.push_back({ (uint64_t)mbi.BaseAddress, (uint64_t)mbi.RegionSize });
        }
...
static bool IsInRegions(uint64_t v, const std::vector<mod_region_t>& regions)
{
    size_t lo = 0, hi = regions.size();
    while (lo < hi)
    {
        size_t mid = (lo + hi) / 2;
        if (v < regions[mid].start) hi = mid;
        else if (v >= regions[mid].start + regions[mid].size) lo = mid + 1;
        else return true;
    }
    return false;
}

static bool CollectFdmCandidates(uint64_t base, uint64_t size,
    const std::vector<mod_region_t>& regions, std::vector<uint64_t>& slots)
{
    const uint64_t kChunk = 0x10000;
    std::vector<uint8_t> buf(kChunk);
    std::unordered_set<uint64_t> seen;
    for (uint64_t off = 0; off < size; off += kChunk)
    {
        uint64_t to_read = std::min<uint64_t>(kChunk, size - off);
        if (memory->read_raw(base + off, buf.data(), (ULONG)to_read) < 0) continue;
        for (uint64_t i = 0; i + 8 <= to_read; i += 8)
        {
            uint64_t v;
            memcpy(&v, buf.data() + i, 8);
            if (v < 0x10000 || (v >> 47) != 0) continue;
            if (!IsInRegions(v, regions)) continue;
            if (seen.insert(v).second)
                slots.push_back(base + off + i);
            if (slots.size() > 250000) return false; // give up on huge sections
        }
    }
    return true;
}

static uint64_t ValidateFdmSlots(const std::vector<uint64_t>& slots)
{
    size_t checked = 0;
    for (uint64_t slot : slots)
    {
        uint64_t fdm = memory->read<uint64_t>(slot);
        if (fdm < 0x10000 || (fdm >> 47) != 0) continue;
        uint64_t rdm = memory->read<uint64_t>(fdm + Offsets::FakeDataModel::RealDataModel);
        if (rdm < 0x10000 || (rdm >> 47) != 0) continue;
        rbx::instance_t dm(rdm);
        if (dm.find_first_child_by_class("Players").address != 0)
        {
            Console::Debug("[FDM-SCAN] slot=0x%llX fdm=0x%llX datamodel=0x%llX", slot, fdm, rdm);
            return slot;
        }
        if (++checked > 150000) break;
    }
    return 0;
}

static uint64_t FindFdmSlotInModule(uint64_t mod_base, uint32_t size_of_image)
{
    if (!size_of_image) return 0;
    std::vector<mod_region_t> regions = BuildReadableRegions();
    if (regions.empty()) return 0;

    uint32_t e_lfanew = memory->read<uint32_t>(mod_base + 0x3C);
    if (!e_lfanew || e_lfanew >= size_of_image) return 0;
    uint16_t nsec = memory->read<uint16_t>(mod_base + e_lfanew + 0x06);
    uint16_t opt_size = memory->read<uint16_t>(mod_base + e_lfanew + 0x14);
    uint64_t sec_start = mod_base + e_lfanew + 0x18 + opt_size;

    std::vector<uint64_t> slots;
    for (uint32_t s = 0; s < nsec; s++)
    {
        uint64_t sec = sec_start + (uint64_t)s * 40;
        uint32_t vsize = memory->read<uint32_t>(sec + 0x08);
        uint32_t vaddr = memory->read<uint32_t>(sec + 0x0C);
        uint32_t chars = memory->read<uint32_t>(sec + 0x24);
        if (!vsize || !vaddr) continue;
        if (vaddr >= size_of_image) continue;
        if (vaddr + vsize > size_of_image) vsize = size_of_image - vaddr;
        if (chars & 0x20000000) continue; // executable section (skip code)
        if (!CollectFdmCandidates(mod_base + vaddr, vsize, regions, slots))
            return 0;
    }
    Console::Debug("[FDM-SCAN] scanned %zu candidates in module 0x%llX", slots.size(), mod_base);
    return ValidateFdmSlots(slots);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, PSTR, int)
{
    try
    {
        ...
        std::thread([]()
        {
            ...
            Console::Info("[+] Found module: 0x%llX (%s)", memory->get_module_address(), which_mod);

            Console::Info("[*] Auto-updating offsets...");
            AutoUpdater::update_all(memory->get_process_handle());
            AutoUpdater::validate_version(memory->get_process_handle());
            ...
            // ── Enumerate all modules and pick the game module: the one whose
            //    image size actually covers the FDM/VE pointer RVAs from the dump ──
            struct mod_info { std::string name; uint64_t base; uint32_t size; };
            ...
            {
                HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, memory->get_process_id());
                if (snap != INVALID_HANDLE_VALUE)
                {
                    ...
                    if (Module32First(snap, &me))
                    {
                        do
                        {
                            uint64_t mbase = (uint64_t)me.modBaseAddr;
                            uint32_t lfanew = memory->read<uint32_t>(mbase + 0x3C);
                            uint32_t soi = lfanew ? memory->read<uint32_t>(mbase + lfanew + 0x50) : 0;
                            Console::Debug("[MOD] %-24s base=0x%llX size=0x%X (%u MB)",
                                me.szModule, mbase, soi, soi / (1024 * 1024));
                            mods.push_back({ me.szModule, mbase, soi });
                            if (!lstrcmpiA(me.szModule, "RobloxPlayerBeta.exe")) base_exe = mbase;
                            if (soi &&
                                Offsets::FakeDataModel::Pointer < soi &&
                                Offsets::VisualEngine::Pointer < soi &&
                                soi > best_fit_size)
                            {
                                best_fit_size = soi;
                                base_dll = mbase;
                                game_mod_name = me.szModule;
                            }
            ...
            if (!game_mod_name.empty())
            {
                memory->find_module_address(game_mod_name); // make get_module_address() return the game module
                Console::Info("[+] Game module '%s' @ 0x%llX (SizeOfImage=0x%X) - offsets fit", game_mod_name.c_str(), base_dll, best_fit_size);
            }
            else
            {
                base_dll = memory->get_module_address();
                Console::Warn("[!] No loaded module fits FDM/VE offset RVAs - will scan for FDM slot");
            }
            Console::Info("[+] Base: 0x%llX (%s)", base_dll, game_mod_name.empty() ? "default" : game_mod_name.c_str());
            ...
            while (true)
            {
                ...
                // Fallback: static FDM/VE offsets failed (wrong module / regenerated
                // dump). Scan loaded modules for the FDM pointer slot by validating
                // the FDM -> RealDataModel -> Players chain, then override the offset.
                if (sec == 30) {
                    Console::Warn("[!] Static offsets failed - scanning modules for FDM slot...");
                    uint64_t slot = 0;
                    for (auto& mi : mods) {
                        if (!mi.size) continue;
                        slot = FindFdmSlotInModule(mi.base, mi.size);
                        if (slot) {
                            Console::Info("[+] FDM slot found in '%s' (slot=0x%llX)", mi.name.c_str(), slot);
                            base_dll = mi.base;
                            memory->find_module_address(mi.name);
                            Offsets::FakeDataModel::Pointer = slot - mi.base;
                            Console::Info("[+] Overrode FDM.Pointer = 0x%llX", Offsets::FakeDataModel::Pointer);
                            break;
                        }
                    }
                    if (!slot) Console::Error("[-] FDM slot not found in any module");
                }
