...
static void TabKeybinds()
{
    ...
    ImGui::SeparatorText("Jump Power");
    {
        bool jp_combined = settings::movement::jump_power::enabled || football::is_jump_power_keybind_active();
        if (ImGui::Checkbox("Enable##jp", &jp_combined))
            settings::movement::jump_power::enabled = jp_combined;
    }
    ImGui::Text("Jump Power");
    ImGui::SetNextItemWidth(-1.0f);
    ImGui::SliderFloat("##jp_value", &settings::movement::jump_power::value, 1.0f, 200.0f, "%.1f");
    ImGui::Text("Jump Cooldown");
    ImGui::SetNextItemWidth(-1.0f);
    ImGui::SliderFloat("##jp_cd", &settings::movement::jump_power::jump_cd, 0.0f, 5.0f, "%.2fs");
    ImGui::Text("Keybind");
    ImGui::SameLine();
    keybind::keybind_selector("##jp_key", &settings::movement::jump_power::keybind,
        &settings::movement::jump_power::activation_mode);
...
static void TabRottedDive()
{
    ImGui::Checkbox("Enable Rotted Dive", &settings::football::rotdive_enabled);
    ImGui::Text("Keybind");
    ImGui::SameLine();
    keybind::keybind_selector("##rotdive_key", &settings::football::rotdive_key, &settings::football::rotdive_key_mode);
    ImGui::Spacing();

    ImGui::SeparatorText("Settings");
    ImGui::Text("Leap Key");
    ImGui::SameLine();
    keybind::keybind_selector("##rotdive_leap_key", &settings::football::rotdive_leap_key, nullptr);
...
static void TabFootballLocked()
{
    if (ImGui::BeginTabBar("FootballTabs", ImGuiTabBarFlags_NoTooltip))
    {
        ...
        if (ImGui::BeginTabItem("Rotted Dive"))
        {
            TabRottedDive();
            ImGui::EndTabItem();
        }
