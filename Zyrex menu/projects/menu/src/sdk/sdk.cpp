...
{
	std::uint64_t name_container = memory->read<std::uint64_t>(this->address + Offsets::Instance::NameContainer);
	...
	return memory->read_string(name_container + Offsets::Instance::Name);
}
