...
static void TabAutoGK()
{
	...
	ImGui::SliderFloat("Jump Delay (TOP zones)", &settings::football::dive_jump_delay, 0.0f, 0.15f, "%.3f");
	ImGui::Spacing();
...
static void TabRottedDive()
{
	...
	keybind::keybind_selector("Leap Key", &settings::football::rotdive_leap_key, nullptr);
	...
	ImGui::Text("Zones LEFT/MID_LEFT = A, RIGHT/MID_RIGHT = D, MIDDLE = random");
	ImGui::Text("Releases wrong direction, taps correct, then leaps");
}
