...
namespace football
{
    ...
    static bool g_jp_kb_active{ false };
    static double g_rotdive_cd{ 0.0 };
    static bool g_rotdive_kb_active{ false };

    // ── Jump Power background thread ──────────────────────────────────────
    static std::atomic<bool> g_jp_thread_running{ false };
    static std::atomic<bool> g_jp_thread_stop{ false };
    static std::thread       g_jp_thread;
    static std::atomic<uintptr_t> g_jp_hum_addr{ 0 };
    static std::atomic<float>     g_jp_value{ 109.8f };
    static std::atomic<float>     g_jp_jh_value{ 34.7f };

    static void jp_thread_func()
    {
        double last_jump_time = 0.0;
        bool prev_jump = false;

        while (!g_jp_thread_stop.load()) {
            bool jp_active = g_jp_thread_running.load();
            uintptr_t hum = g_jp_hum_addr.load();
            if (jp_active && hum != 0) {
                float jp = g_jp_value.load();
                float jh = g_jp_jh_value.load();
                memory->write<float>(hum + Offsets::Humanoid::JumpHeight, jh);
                memory->write<float>(hum + Offsets::Humanoid::JumpPower, jp);
                memory->write<uint8_t>(hum + Offsets::Humanoid::UseJumpPower, 1);

                float cd = settings::movement::jump_power::jump_cd;
                if (cd > 0.0f) {
                    uint8_t jump_val = memory->read<uint8_t>(hum + Offsets::Humanoid::Jump);
                    bool jumping = (jump_val != 0);

                    auto now = std::chrono::steady_clock::now();
                    double now_s = std::chrono::duration<double>(now.time_since_epoch()).count();

                    if (jumping && !prev_jump) {
                        if (now_s - last_jump_time < cd) {
                            memory->write<uint8_t>(hum + Offsets::Humanoid::Jump, 0);
                        } else {
                            last_jump_time = now_s;
                        }
                    }
                    prev_jump = jumping;
                }
    ...
    static void jp_thread_start()
    {
        if (g_jp_thread.joinable()) return;
        g_jp_thread_stop.store(false);
        g_jp_thread = std::thread(jp_thread_func);
    }

    static void jp_thread_stop_fn()
    {
        g_jp_thread_stop.store(true);
        g_jp_thread_running.store(false);
        if (g_jp_thread.joinable())
            g_jp_thread.join();
    }
    ...
    static void do_dive(const std::string& zone, bool use_random_key = false)
    {
        ...
        bool do_jump = (row == "TOP");

        // Direction key based on column
        WORD dir_key = 0;
        if (col == "LEFT" || col == "MID_LEFT")
            dir_key = (WORD)settings::football::dive_key_left;
        else if (col == "RIGHT" || col == "MID_RIGHT")
            dir_key = (WORD)settings::football::dive_key_right;
        else if (col == "MIDDLE" && settings::football::dive_key_middle != 0)
            dir_key = (WORD)settings::football::dive_key_middle;

        // Random alternate direction key
        if (use_random_key && settings::football::random_dive_enabled) {
            ...
            if (all_keys.size() > 1) {
                dir_key = all_keys[rand() % all_keys.size()];
            }
        }
        ...
        if (do_jump) {
            ...
            if (settings::football::dive_jump_delay > 0.0f)
    ...
    static void do_rotted_dive(const std::string& zone, const math::vector3& hit)
    {
        ...
        up_dir.ki.dwFlags = KEYEVENTF_SCANCODE | KEYEVENTF_KEYUP;
        SendInput(1, &up_dir, sizeof(INPUT));

        g_rotdive_cd = g_now + (double)settings::football::rotdive_cooldown;
    }
    ...
    static void process_keybinds()
    {
        ...
        // Auto M2
        {
            ...
            kb.mode = (keybind::activation_mode)settings::football::auto_m2_key_mode;
            if (kb.key != 0)
        ...
        {
            static keybind::keybind_t kb;
            kb.key = settings::movement::jump_power::keybind;
            kb.mode = (keybind::activation_mode)settings::movement::jump_power::activation_mode;
            if (kb.key != 0)
                g_jp_kb_active = keybind::is_active(kb);
            else
                g_jp_kb_active = false;
        }
        // Rotted Dive keybind
        {
            static keybind::keybind_t kb;
            kb.key = settings::football::rotdive_key;
            kb.mode = (keybind::activation_mode)settings::football::rotdive_key_mode;
            if (kb.key != 0)
                settings::football::rotdive_enabled = keybind::is_active(kb);
        }
    }
    ...
    void tick()
    {
        if (memory->get_process_id() == 0 || memory->get_process_handle() == nullptr ||
            memory->get_process_handle() == INVALID_HANDLE_VALUE) {
        ...
        // ── Jump Power (background thread) ────────────────────────────────
        {
            ...
            if (jp_active && !prev_jp_active && humanoid.address != 0) {
                ...
                g_jp_thread_running.store(true);
                jp_thread_start();
            }
            ...
            prev_jp_active = jp_active;
        }

        bool en = settings::football::autodive_enabled || settings::football::auto_m2 || settings::football::rotdive_enabled;
        ...
        for (auto& ball : balls)
        {
            ...
            if (valid_pred || is_tracked)
            {
                ...
                if (valid_pred)
                {
                    ...
                    if (settings::football::rotdive_enabled && ttg <= offset && g_now >= g_rotdive_cd)
                        do_rotted_dive(final_zone, hit);
                }
                else
                {
    ...
    bool is_jump_power_keybind_active() { return g_jp_kb_active; }
} // namespace football
