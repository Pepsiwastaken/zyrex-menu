...
int AutoUpdater::update_offsets(const std::string& version)
{
    ...
    if (!version.empty())
    {
        std::string versioned_url = "https://offsets.imtheo.lol/offsets/" + version + "/Offsets.hpp";
        Console::Info("[AutoUpdater] Fetching versioned offsets (%s)...", version.c_str());
        if (download_to_string(versioned_url.c_str(), content))
        {
            Console::Info("[AutoUpdater] Versioned dump OK (%zu bytes)", content.size());
        }
        else
        {
            Console::Warn("[AutoUpdater] No versioned dump for %s, falling back to latest", version.c_str());
        }
    }

    if (content.empty() && !download_to_string("https://offsets.imtheo.lol/offsets.hpp", content))
    {
...
void AutoUpdater::update_all(HANDLE process_handle)
{
    ...
    if (process_handle && process_handle != INVALID_HANDLE_VALUE)
        running = OffsetLoader::GetRobloxVersionFromProcess(process_handle, 0);

    if (!running.empty())
        Console::Info("Targeting running Roblox version: %s", running.c_str());

    update_offsets(running);
    Console::Info("Offsets updated at runtime.");
    Console::Info("====================");
}

void AutoUpdater::validate_version(HANDLE process_handle)
{
    std::string running = OffsetLoader::GetRobloxVersionFromProcess(process_handle, 0);
    std::string expected = OffsetLoader::GetExpectedVersion();
    if (expected.empty())
        expected = Offsets::ClientVersion;

    if (expected.empty())
    {
        Console::Warn("[AutoUpdater] No version info to compare against");
        return;
    }

    if (running.empty())
    {
        Console::Warn("[AutoUpdater] Could not determine running Roblox version (unusual install path?)");
        return;
    }

    if (running != expected)
    {
        Console::Warn("[AutoUpdater] VERSION MISMATCH");
        Console::Warn("  Running  : %s", running.c_str());
        Console::Warn("  Offsets  : %s", expected.c_str());
        Console::Warn("  Features may not work correctly until the offsets match this Roblox version");
    }
    else
    {
        Console::Info("[AutoUpdater] Version OK: %s", running.c_str());
    }
}
